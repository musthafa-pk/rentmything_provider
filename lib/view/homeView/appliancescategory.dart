...
class AppliancesCategory extends StatefulWidget {
  String category;
  AppliancesCategory({required this.category,super.key});

  @override
  State<AppliancesCategory> createState() => _AppliancesCategoryState();
}

class _AppliancesCategoryState extends State<AppliancesCategory> {


  List<dynamic> appliancescategory = [];

  Future<dynamic> getappliances() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(responseData);
          appliancescategory.clear();
          appliancescategory.addAll(responseData['data']);
            ...
            child: FutureBuilder(
              ...
              builder: (context,snapshot) {
                if(snapshot.connectionState == ConnectionState.waiting){
                  ...
                  return const Center(child: Text('Some error happened !',style: TextStyle(color: AppColors.color1),),);
                }if(snapshot.hasData){
                  return ListView.builder(
                    itemCount: appliancescategory.length,
                                        ...
                                        child: Container(
                                          ...
                                          decoration: BoxDecoration(
                                            ...
                                            image: DecorationImage(
                                              image: NetworkImage(
                                                snapshot.data['data'][index]['image'] != null && snapshot.data['data'][index]['image'].isNotEmpty ? snapshot.data['data'][index]['image'][0] : 'https://via.placeholder.com/150', // Display the first image if available, otherwise display a placeholder image
                                              ),
                                              fit: BoxFit.cover,
                                            ),
                                          ),
                                          ...
                                          children: [
                                            Row(
                                              children: [
                                                ...
                                                RentTypeWidget(listofProducts: appliancescategory, index: index, renttype: appliancescategory[index]['time_period']),
                                              ],
                                            ),
